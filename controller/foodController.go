...
func CreateFood() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Try to bind to a slice first (multiple foods)
		...
		if err := c.BindJSON(&foods); err == nil {
			...
			for i := range foods {
				...
				err := menuCollection.FindOne(ctx, bson.M{"menu_id": foods[i].Menu_id}).Decode(&menu)
				if err != nil {
				...
				now := time.Now()
				foods[i].Created_at = now
				foods[i].Updated_at = now

				if foods[i].Price != nil {
					num := toFixed(*foods[i].Price, 2)
					foods[i].Price = &num
				}

				docs = append(docs, foods[i])
			}

			result, err := foodCollection.InsertMany(ctx, docs)
			if err != nil {
				c.JSON(http.StatusInternalServerError, gin.H{"error": "could not insert foods"})
				return
			}
		...
		// If binding to slice fails, try single food
		var food models.Food
